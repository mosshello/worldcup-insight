...
</style><link rel="stylesheet" href="flat.css"></head><body><main class="page"><header class="hero"><div><p class="eyebrow">WORLD CUP INSIGHT · DREAMV</p><h1>世界杯自动策略台</h1><p class="subtitle">模型候选 · 组合 SP ≥ 2.00 · 累计盈亏 · 历史账本</p></div><div class="meta"><div><span>运行状态</span><strong>每日北京时间 20:05</strong></div><div><span>最新账本</span><strong>{_escape(latest.get("recorded_at"))}</strong></div></div></header><section class="summary"><div><span>累计总投入</span><strong>¥{summary['total_invested']:.2f}</strong></div><div><span>已结算盈利</span><strong class="profit">¥{summary['realized_profit']:+.2f}</strong></div><div><span>未结算潜在盈利</span><strong>¥{summary['open_potential_profit']:.2f}</strong></div><div><span>历史方案</span><strong>{summary['entry_count']} 天</strong></div></section>{panel}<section class="panel"><div class="panel-head"><div><p class="eyebrow">HISTORY</p><h2>历史记录</h2><p>待结算记录不计入已实现盈利。</p></div></div><div class="table-wrap"><table><thead><tr><th>日期</th><th>总投入</th><th>二串一</th><th>组合 SP</th><th>状态</th><th>实际盈亏</th></tr></thead><tbody>{history}</tbody></table></div></section><section class="panel"><h2>自动运行口径</h2><p class="note">稳健单场取模型候选中去水概率最高项；二串一枚举模型高信心且内外盘同向方向，只保留组合 SP 不低于 2.00 的组合，再选择联合概率最高项。你不需要每天修改。</p><p class="note">当前没有合法投注平台下单 API，因此不会真实扣款或自动购彩。待结算数据不会提前计入盈利。</p></section><footer>仅供数据分析与模拟记账，不构成投注建议；历史表现不保证未来结果。</footer></main></body></html>'''
