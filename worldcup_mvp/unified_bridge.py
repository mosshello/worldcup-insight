...
from .data_manager import ConfigurationError, UnifiedDataManager
...
def get_provider_health() -> dict[str, Any]:
    """检查 FIFA / Polymarket / 体彩三源可用性（失败时不抛错）。"""
    try:
        ...
        return {"success": False, "error": str(exc), "providers": [], "all_ok": False}
...
def load_unified_index(
    ...
    *,
    ...
    global _index_cache, _index_date
    ...
    for match in payload.get("matches", []):
        ...
        if sporttery_id:
            by_sporttery_id[str(sporttery_id)] = match
...
def get_unified_match(sporttery_match_id: str) -> dict[str, Any] | None:
    """按体彩 match_id 获取三源统一比赛对象。"""
    index = load_unified_index()
...
def build_context_analysis(unified_match: dict[str, Any]) -> dict[str, Any] | None:
    """对三源统一比赛运行增强版 analyzer（含 FIFA 上下文）。"""
    try:
        return analyze_match(unified_match)
    except ValueError:
        return None
